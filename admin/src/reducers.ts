...
export function getFieldState(state: any, field: any) {
  const { uid, name, locale } = field;
  return (
    state[CONSTANTS.REDUCER]?.[locale || DEFAULT_LOCALE]?.[uid]?.[name] || {}
  );
}

function updateFieldState(state: any, field: any, data: any) {
