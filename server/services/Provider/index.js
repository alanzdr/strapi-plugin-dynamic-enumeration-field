...
const dynamicEnumProvider = ({strapi}) => ({
  async getEnumeration ({ uid, name, locale }) {
    const response = await strapi.db.query(contentType).findOne({ // uid syntax: 'api::api-name.content-type-name'
      where: {
        uid,
        name,
        ...(locale ? { locale } : {})
  ...
  async queryFields ({ uid, namesIn, locale }) {
    const response = await strapi.db.query(contentType).findMany({ // uid syntax: 'api::api-name.content-type-name'
      where: {
        uid,
        name: { $in: namesIn },
        ...(locale ? { locale } : {})
...
module.exports = dynamicEnumProvider