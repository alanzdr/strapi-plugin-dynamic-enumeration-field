import LifeCycleHandle from "./handle";
import LifeCycleQuery from "./query";
...
async function executeLifecycle(event) {
  ...
  // Get field names
  const fieldNames = Object.entries(attributes)
    .filter(([key, value]) => {
...
export default executeLifecycle;
